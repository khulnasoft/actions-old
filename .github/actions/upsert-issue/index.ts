...
async function updateIssuePlan(issue: any, changes: Change[], title: string) : Promise<any> {
  ...
  // Currently, Khulnasoft only allows to in-place update existing spec in the plan steps. And it
  ...
  for (const change of changes) {
    ...
    for (const step of planData.steps) {
      ...
      if (!matchedSpec) {
        throw new Error(`Khulnasoft disallow adding new migration file to the existing issue: ${change.file}`);
      }
    }
  }
  ...
  for (const step of planData.steps) {
    for (const spec of step.specs) {
      ...
      if (!matchedSpec) {
        throw new Error('Khulnasoft disallow removing migration file from the existing issue.');
      }
    }
  }
